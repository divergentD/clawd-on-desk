...
<p align="center">
  <img src="assets/hero.gif" alt="望仔 桌宠动画演示：像素螃蟹会随 AI 编程助手状态实时切换，在睡觉、思考、工具运行时打字、单个子代理时戴耳机律动、多个子代理并行时三球杂耍、权限请求弹出时提醒、任务完成后庆祝。支持 Claude Code、Codex、Cursor、Copilot、Gemini、Antigravity、Qwen、Pi、OpenClaw 等。">
</p>

望仔 住在你的桌面上，实时感知 AI 编程助手正在做什么。发起一个长任务，起身去做点别的，等螃蟹告诉你任务完成了再回来。

你提问时它思考，工具运行时它打字，子代理工作时它会戴耳机律动或三球杂耍，审批权限时它弹卡片，任务完成时它庆祝，你离开时它睡觉。内置三套主题：**望仔**（像素螃蟹）、**Calico**（三花猫）和 **Cloudling**（云宝），支持自定义主题，也支持导入 Codex Pet 动画包。
...
- **Gemini CLI** — 通过 `~/.gemini/settings.json` 配置 command hook（望仔 启动时自动注册，或执行 `npm run install:gemini-hooks`）
- **Antigravity CLI (agy)** — 通过 `~/.gemini/config/hooks.json` 配置 command hook（已有 Antigravity 配置时 望仔 启动会自动注册，或执行 `npm run install:antigravity-hooks`）；**仅状态同步**：望仔 不会为 agy 弹任何权限气泡，所有 Allow / Deny / Always-allow 都在 agy 自己的终端菜单里完成
- **Cursor Agent** — [Cursor IDE hooks](https://cursor.com/docs/agent/hooks)，配置在 `~/.cursor/hooks.json`（望仔 启动时自动注册，或执行 `npm run install:cursor-hooks`）
- **CodeBuddy** — 通过与 Claude Code 兼容的 command hook + HTTP 权限 hook 集成，配置写入 `~/.codebuddy/settings.json`（望仔 启动时自动注册，或执行 `node hooks/codebuddy-install.js`）
- **Kiro CLI** — command hooks 注入到 `~/.kiro/agents/` 下的自定义 agent 配置中，并自动创建一个 `wang-pet` agent；望仔 每次启动时都会重新从内置 `kiro_default` 同步它，尽量保持与默认 agent 一致。macOS 与 Windows 上状态动效已验证可用；需要时可用 `kiro-cli --agent 望仔` 或在会话内执行 `/agent swap 望仔` 启用 hooks（望仔 启动时自动注册，或执行 `npm run install:kiro-hooks`）
- **Kimi Code CLI（Kimi-CLI）** — 通过 `~/.kimi/config.toml`（`[[hooks]]` 条目）配置 command hooks（望仔 启动时自动注册，或执行 `npm run install:kimi-hooks`）
- **Qwen Code** — 通过 `~/.qwen/settings.json` 配置 command hooks（望仔 启动时自动注册，或执行 `npm run install:qwen-hooks`）；支持状态追踪和 Qwen `PermissionRequest` 桌面权限气泡
- **opencode** — [plugin 集成](https://opencode.ai/docs/plugins)，写入 `~/.config/opencode/opencode.json`（望仔 启动时自动注册）；零延迟事件流、Allow/Always/Deny 权限气泡、`task` 工具分派并行子代理时自动播放建筑动画
- **Pi** — 通过全局 extension 集成，写入 `~/.pi/agent/extensions/wang-pet`（望仔 启动时自动注册，或执行 `npm run install:pi-extension`）；仅同步交互式 Pi 会话生命周期和工具活动状态，并保留 Pi 默认 YOLO 行为
- **OpenClaw** — 通过 `~/.openclaw/openclaw.json` 中的 plugin 路径做状态感知（OpenClaw 配置已存在时 望仔 启动会自动注册，或执行 `npm run install:openclaw-plugin`）；Phase 1 面向本地 `openclaw tui --local` 会话，只驱动动画，不接权限气泡和终端聚焦
- **Hermes Agent** — [plugin 集成](https://hermes-agent.org/)，写入 Hermes 的托管 plugin 目录（检测到 Hermes 后 望仔 启动时自动注册，或执行 `npm run install:hermes-plugin`）；支持状态、会话、SessionEnd 和终端聚焦
- **多 Agent 共存** — 多个 Agent 可同时运行，望仔 独立追踪每个会话

### 动画与交互
- **实时状态感知** — 通过 Agent hook 和日志轮询自动驱动动画
- **12 种动画状态** — 待机、思考、打字、建造、耳机律动、多子代理三球杂耍、报错、开心、通知、扫地、搬运、睡觉
- **Codex Pet 导入** — 在 `设置…` → `主题` 中导入 Codex Pet zip 包，望仔 会把 atlas 动画适配成可管理主题
- **眼球追踪** — 待机状态下 望仔 跟随鼠标，身体微倾，影子拉伸
- **睡眠序列** — 60 秒无活动 → 打哈欠 → 打盹 → 倒下 → 睡觉；移动鼠标触发惊醒弹起动画
- **点击反应** — 双击戳戳，连点 4 下东张西望
- **任意状态拖拽** — 随时抓起 望仔（Pointer Capture 防止快甩丢失），松手恢复当前动画
- **极简模式** — 拖到右边缘或右键"极简模式"；望仔 藏在屏幕边缘，悬停探头招手，通知/完成有迷你动画，抛物线跳跃过渡

### 权限审批气泡
- **桌面端权限审批** — 当 Claude Code、Codex CLI、CodeBuddy 或 opencode 请求受支持的工具权限时，望仔 会弹出浮动卡片，无需切回终端
...
- **会话 Dashboard + HUD** — 右键或托盘 → `打开 Dashboard` 查看活跃会话、最近事件、别名，并可跳转终端；望仔 附近的轻量 HUD 会持续显示当前 live session
- **终端聚焦** — Dashboard / HUD 操作可跳转到指定会话的终端窗口；通知/注意状态会自动聚焦相关终端
- **进程存活检测** — 检测已崩溃/退出的受支持 Agent 进程，并在 10 秒内清理孤儿会话
- **启动恢复** — 如果 望仔 重启时仍有受支持的 Agent 在运行，它会保持清醒等待后续事件，而不是直接睡觉

### 系统
- **点击穿透** — 透明区域的点击直接穿透到下方窗口，只有角色本体可交互
- **位置记忆** — 重启后 望仔 回到上次的位置（包括极简模式）
- **单实例锁** — 防止重复启动
- **自动启动** — Claude Code 的 SessionStart hook 可在 望仔 未运行时自动拉起
...
<table>
  <tr>
    <td align="center"><img src="assets/gif/wang-pet-idle.gif" width="100"><br><sub>待机</sub></td>
    <td align="center"><img src="assets/gif/wang-pet-thinking.gif" width="100"><br><sub>思考泡泡</sub></td>
    <td align="center"><img src="assets/gif/wang-pet-typing.gif" width="100"><br><sub>打字</sub></td>
    <td align="center"><img src="assets/gif/wang-pet-building.gif" width="100"><br><sub>建造</sub></td>
    <td align="center"><img src="assets/gif/wang-pet-headphones-groove.gif" width="100"><br><sub>耳机律动</sub></td>
    <td align="center"><img src="assets/gif/wang-pet-juggling.gif" width="100"><br><sub>三球杂耍</sub></td>
  </tr>
  <tr>
...
望仔 适配多显示器场景：按启动时所在显示器做等比缩放，竖屏显示器有尺寸加成防止宠物过小，也可以跨屏拖动。

<p align="center"><sub>想看多显示器下的实际效果？可以<a href="assets/videos/wang-pet-multi-monitor-demo.mp4">打开仓库里的演示视频</a>。</sub></p>

## 快速开始

普通用户建议直接从 **[GitHub Releases](https://github.com/rullerzhou-afk/wang-pet/releases/latest)** 下载最新预构建安装包：

- **Windows**：`WangPet-Setup-<version>-x64.exe` 或 `WangPet-Setup-<version>-arm64.exe`
- **macOS**：`.dmg`
- **Linux**：`.AppImage` 或 `.deb`

安装后启动 望仔；支持的 agent hooks / plugins 会在启动时自动同步。
...
# 启动 望仔（启动时会自动注册 Claude Code hooks；如需预先手动注册，可单独执行 `node hooks/install.js`）
npm start
```

**Claude Code**、**Codex CLI**、**Copilot CLI** 会自动注册 hooks，开箱即用。**Gemini CLI**、**Antigravity CLI (agy)**、**Cursor Agent**、**CodeBuddy**、**Kiro CLI**、**Kimi Code CLI（Kimi-CLI）**、**Qwen Code**、**opencode**、**Pi**、**OpenClaw**、**Hermes Agent** 在已安装前提下会在 望仔 启动时自动同步（OpenClaw 还需要已有配置）。也涵盖远程 SSH、WSL 及平台说明（macOS / Linux）：**[docs/guides/setup-guide.zh-CN.md](docs/guides/setup-guide.zh-CN.md)**

想在远程服务器上跑 Claude Code / Codex CLI 并把状态和权限气泡转发到本地 望仔？应用内 **Settings → 远程 SSH → 一键部署** 即可。完整步骤、Doctor 边界和 FAQ 见：**[docs/guides/guide-remote-ssh.zh-CN.md](docs/guides/guide-remote-ssh.zh-CN.md)**

关于 `Codex + WSL` 的官方现状、望仔 当前实现边界、以及为什么容易被误解，见：**[docs/guides/codex-wsl-clarification.zh-CN.md](docs/guides/codex-wsl-clarification.zh-CN.md)**
...
望仔 支持自定义主题——用你自己的角色和动画替换默认的螃蟹。如果你已有 Codex Pet 包，也可以在 `设置…` → `主题` → `导入宠物 zip` 直接导入，望仔 会自动把 atlas 转成托管主题。
...
望仔 是一个社区驱动的项目。欢迎提 Bug、提需求、提 PR —— 在 [Issues](https://github.com/rullerzhou-afk/wang-pet/issues) 里聊或直接提交 PR。
...
- 望仔 像素画参考自 [wang-pet-tank](https://github.com/marciogranzotto/wang-pet-tank) by [@marciogranzotto](https://github.com/marciogranzotto)
...
- **望仔** 角色设计归属 [Anthropic](https://www.anthropic.com)。本项目为非官方粉丝作品，与 Anthropic 无官方关联。

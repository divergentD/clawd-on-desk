#!/usr/bin/env node
// WangPet - Antigravity CLI hook adapter
...
function getAntigravityPermissionTimeoutMs(env = process.env) {
  const raw = Number(env.WANGPET_ANTIGRAVITY_PERMISSION_TIMEOUT_MS);
...
function isAntigravityHookDebugEnabled(env = process.env) {
  return isTruthyDebugValue(String(env.WANGPET_ANTIGRAVITY_HOOK_DEBUG || "").toLowerCase());
}

function getAntigravityHookDebugLogPath(env = process.env) {
  if (typeof env.WANGPET_ANTIGRAVITY_HOOK_DEBUG_FILE === "string" && env.WANGPET_ANTIGRAVITY_HOOK_DEBUG_FILE.trim()) {
    return env.WANGPET_ANTIGRAVITY_HOOK_DEBUG_FILE.trim();
  }
  return path.join(os.homedir(), ".gemini", "antigravity-cli", "wang-pet-hook-debug.log");
}
...
function writeAntigravityHookDebug(env, event, fields = {}) {
  ...
  if (isTruthyDebugValue(String(env.WANGPET_ANTIGRAVITY_HOOK_DEBUG_STDERR || "").toLowerCase())) {
    try {
      process.stderr.write(`[wang-pet-antigravity] ${line}\n`);
    } catch {}
  }

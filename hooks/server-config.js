...
const HOST_PREFIX_PATH = path.join(os.homedir(), ".claude", "hooks", "wang-pet-host-prefix");
...
function writeRuntimeConfig(port) {
  ...
  const body = JSON.stringify({ app: WANGPET_SERVER_ID, port: safePort }, null, 2);
...
function isRemoteHookMode(options = {}) {
  ...
  const value = env && env.WANGPET_REMOTE;
...
function getPermissionProbeTimeoutMs(options = {}) {
  ...
  // healthy local WangPet behind a high-latency tunnel.
  return normalizeHookHttpTimeout(
...
function probePort(port, timeoutMs, callback, options = {}) {
  ...
  const req = httpGet(
    ...
    (res) => {
      ...
      res.on("end", () => callback(iswangpetResponse(res, body)));
    }
  );
...
function postStateToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      if (readHeader(res, WANGPET_SERVER_HEADER) === WANGPET_SERVER_ID) {
        res.resume();
        callback(true, port);
        return;
      }
      ...
      res.on("end", () => callback(iswangpetResponse(res, responseBody), port));
    }
  );
...
function discoverwangpetPort(options, callback) {
...
function postPermissionToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      ...
      res.on("end", () => {
        finish(readHeader(res, WANGPET_SERVER_HEADER) === WANGPET_SERVER_ID, responseBody, res.statusCode || 0);
      });
    }
  );
...
function postPermissionToRunningServer(body, options, callback) {
  ...
  const payload = typeof body === "string" ? body : JSON.stringify(body);
  const discover = options && options.discoverwangpetPort ? options.discoverwangpetPort : discoverwangpetPort;
...
function iswangpetOrElectronPath(value) {
  ...
  // contain "wang-pet" or "Electron".
  const base = path.win32.basename(norm);
  return base.includes("WangPet on desk") || base === "electron.exe";
}
...
function validateWindowsNodeCandidate(value) {
  ...
  if (iswangpetOrElectronPath(trimmed)) return null;
  return trimmed;
}
...
function resolveWindowsNodeBinSync(options = {}) {
  ...
  //    In packaged WangPet builds this is `WangPet.exe`, so it falls

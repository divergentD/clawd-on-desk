...
function attach(pi, deps = {}) {
  ...
  for (const [nativeName, wangpetEvent, state] of DEFAULT_EVENT_BINDINGS) {
    const wait = nativeName === "agent_end" || nativeName === "session_shutdown";
    pi.on(nativeName, (nativeEvent, ctx) => send(state, wangpetEvent, nativeEvent, ctx, wait));
  }
  ...
  pi.on("tool_result", (nativeEvent, ctx) => {
    ...
    // the error state before WangPet receives it.
    return send(

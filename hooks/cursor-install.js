#!/usr/bin/env node
// Merge WangPet Cursor Agent hooks into ~/.cursor/hooks.json (append-only, idempotent)
...
/**
 * Register WangPet hooks into ~/.cursor/hooks.json
...
function registerCursorHooks(options = {}) {
  ...
  if (!options.silent) {
    console.log(`WangPet Cursor hooks → ${hooksPath}`);
...
function unregisterCursorHooks(options = {}) {
  ...
  if (!options.silent) console.log(`WangPet Cursor hooks removed: ${removed}`);

#!/usr/bin/env node
// Merge WangPet Qwen Code hooks into ~/.qwen/settings.json.
...
function iswangpetHookCommand(command) {
...
function normalizeQwenCodeHookEntries(entries, desiredCommand, event) {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    if (iswangpetHookCommand(entry.command)) {
      matched = true;
      if (dedicatedIndex === -1) {
    ...
    const otherHooks = [];
    let wangpetHookCount = 0;
    for (const hook of entry.hooks) {
      if (hook && iswangpetHookCommand(hook.command)) wangpetHookCount++;
      else otherHooks.push(hook);
    }
    if (wangpetHookCount === 0) continue;
...
function registerQwenCodeHooks(options = {}) {
  ...
  if (!options.settingsPath && !fs.existsSync(qwenDir)) {
    if (!options.silent) console.log("WangPet: ~/.qwen/ not found - skipping Qwen hook registration");
  ...
  if (settings && settings.disableAllHooks === true) {
    warnings.push("settings.json has disableAllHooks=true; WangPet Qwen hooks will not fire until that flag is removed.");
  }
  ...
  if (!options.silent) {
    console.log(`WangPet Qwen hooks -> ${settingsPath}`);
...
function unregisterQwenCodeHooks(options = {}) {
  ...
  for (const event of QWEN_CODE_HOOK_EVENTS) {
    ...
    const result = removeMatchingCommandHooks(entries, iswangpetHookCommand);
  ...
  if (!options.silent) console.log(`WangPet Qwen hooks removed: ${removed}`);

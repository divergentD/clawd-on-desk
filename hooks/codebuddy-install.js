#!/usr/bin/env node
// Merge WangPet CodeBuddy hooks into ~/.codebuddy/settings.json (append-only, idempotent)
...
/**
 * Register WangPet hooks into ~/.codebuddy/settings.json
...
function registerCodeBuddyHooks(options = {}) {
  ...
  if (!options.settingsPath && !fs.existsSync(codebuddyDir)) {
    if (!options.silent) console.log("WangPet: ~/.codebuddy/ not found — skipping CodeBuddy hook registration");
  ...
  if (!options.silent) {
    console.log(`WangPet CodeBuddy hooks → ${settingsPath}`);
...
function unregisterCodeBuddyHooks(options = {}) {
  ...
  if (!options.silent) console.log(`WangPet CodeBuddy hooks removed: ${removed}`);

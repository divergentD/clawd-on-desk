#!/usr/bin/env node
// Register WangPet's opencode plugin in the user's global opencode config.
//
 ...
 * opencode (Bun) process. When WangPet is packaged into app.asar, hooks/** is
...
/**
 * Register the WangPet opencode plugin in ~/.config/opencode/opencode.json.
 *
...
function registerOpencodePlugin(options = {}) {
  ...
  if (!options.configPath) {
    ...
    if (!exists) {
      if (!options.silent) {
        console.log("WangPet: ~/.config/opencode/ not found — skipping opencode plugin registration");
      }
  ...
  // "/opencode-plugin". WangPet itself only ever writes absolute paths, so
  ...
  if (!options.silent) {
    console.log(`WangPet opencode plugin → ${configPath}`);
...
function unregisterOpencodePlugin(options = {}) {
  ...
  if (!options.silent) console.log(`WangPet opencode plugin entries removed: ${removed}`);

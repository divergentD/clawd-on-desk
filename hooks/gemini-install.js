#!/usr/bin/env node
// Merge WangPet Gemini CLI hooks into ~/.gemini/settings.json (append-only, idempotent)
...
function buildGeminiHookEntry(command) {
  return {
    matcher: "*",
    hooks: [{ name: "wang-pet", type: "command", command }],
  };
}
...
function normalizeGeminiHookEntries(entries, desiredCommand) {
  ...
  for (let index = 0; index < entries.length; index++) {
    ...
    for (const hook of entry.hooks) {
      if (hook && iswangpetHookCommand(hook.command)) {
        wangpetHookCount++;
      } else {
...
function normalizeGeminiDisabledHooks(settings) {
  ...
  for (const entry of hooksConfig.disabled) {
    ...
    if (iswangpetHookCommand(entry)) {
      if (!sawwangpet) {
        nextDisabled.push("wang-pet");
        sawwangpet = true;
      }
      changed = true;
      continue;
...
/**
 * Register WangPet hooks into ~/.gemini/settings.json
...
function registerGeminiHooks(options = {}) {
  ...
  if (!options.settingsPath && !fs.existsSync(geminiDir)) {
    if (!options.silent) console.log("WangPet: ~/.gemini/ not found — skipping Gemini hook registration");
  ...
  if (!options.silent) {
    console.log(`WangPet Gemini hooks → ${settingsPath}`);
...
function unregisterGeminiHooks(options = {}) {
  ...
  if (!options.silent) console.log(`WangPet Gemini hooks removed: ${removed}`);

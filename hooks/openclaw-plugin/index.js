...
export const PLUGIN_ID = "wang-pet";
...
export function createOpenClawRuntime(options = {}) {
  ...
  const postState = typeof options.postState === "function" ? options.postState : postStateTowangpet;
...
export function postStateTowangpet(body) {
...
function postJsonToPort(port, payload) {
  return new Promise((resolve) => {
    ...
    const req = request({
      ...
      const iswangpet = res.headers["x-wang-pet-server"] === "wang-pet";
      res.resume();
      res.on("end", () => finish(iswangpet));
    });
    req.on("timeout", () => {
      req.destroy();
      finish(false);

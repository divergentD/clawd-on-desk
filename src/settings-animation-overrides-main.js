...
const ANIMATION_OVERRIDES_EXPORT_DIALOG_STRINGS = {
  en: {
    ...
    defaultName: (ts) => `wang-pet-animation-overrides-${ts}.json`,
    jsonFilter: "WangPet Animation Overrides",
  ...
  ja: {
    ...
    defaultName: (ts) => `wang-pet-animation-overrides-${ts}.json`,
    jsonFilter: "WangPet アニメーション差し替え",
...
function createSettingsAnimationOverridesMain(options = {}) {
  ...
  function runPendingPostReloadTasks() {
    ...
    for (const task of tasks) {
      try { task(); } catch (err) { console.warn("WangPet: post-reload task threw:", err && err.message); }
    }
  }
  ...
  async function runAnimationPreviewPosterJob(job) {
    try {
      ...
      if (err && err.code === "ANIMATION_PREVIEW_POSTER_TIMEOUT") {
        console.warn("WangPet: animation preview poster capture timed out:", message);
      } else {
        console.warn("WangPet: failed to capture animation preview poster:", message);
      }
    } finally {
  ...
  async function exportAnimationOverrides(event) {
    ...
    try {
      ...
      const payload = {
        wangpetAnimationOverrides: ANIMATION_OVERRIDES_EXPORT_VERSION,
        version: ANIMATION_OVERRIDES_EXPORT_VERSION,
        exportedAt: new Date().toISOString(),
        wangpetVersion: app.getVersion(),
        themes: overrides,
      };
      ...
      console.warn("WangPet: export-animation-overrides failed:", err && err.message);
  ...
  async function importAnimationOverrides(event) {
    ...
    try {
      ...
      console.warn("WangPet: import-animation-overrides dialog failed:", err && err.message);
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return { status: "error", message: "file is not a WangPet animation overrides export" };
    }
    const magic = parsed.wangpetAnimationOverrides;

...
function isEnabledFromEnv(env = process.env) {
  if (truthy(env.WANGPET_HARDWARE_BUDDY_DISABLED)) return false;
  if (truthy(env.WANGPET_HARDWARE_BUDDY)) return true;
  if (env.WANGPET_HARDWARE_BUDDY_BACKEND) return true;
  if (env.WANGPET_HARDWARE_BUDDY_ADDRESS) return true;
  return false;
}
...
function wrapHardwareBuddyTransport(transport) {
  if (!transport || typeof transport.send !== "function" || transport.__wangpetSanitizedSend === true) return transport;
  ...
  Object.defineProperty(transport, "__wangpetSanitizedSend", {
    value: true,
    enumerable: false,
  });
  return transport;
...
function defaultCoreRoot(env = process.env) {
  return env.WANGPET_HARDWARE_BUDDY_ROOT
...
function loadCoreModules(coreRoot) {
  ...
  if (missingPaths.length) {
    const err = new Error(
      "Hardware Buddy core modules are missing. Clone https://github.com/rullerzhou-afk/clawstick next to this repo or set WANGPET_HARDWARE_BUDDY_ROOT."
    );
...
function defaultSidecarScript(coreRoot, env = process.env) {
  return env.WANGPET_HARDWARE_BUDDY_SIDECAR
...
function readRuntimeConfig(options, env = process.env) {
  ...
  if (truthy(env.WANGPET_HARDWARE_BUDDY_DISABLED)) config.enabled = false;
  else if (options.enabled != null) config.enabled = !!options.enabled;
  else if (truthy(env.WANGPET_HARDWARE_BUDDY)) config.enabled = true;

  if (options.permissionsEnabled != null) config.permissionsEnabled = !!options.permissionsEnabled;
  else if (truthy(env.WANGPET_HARDWARE_BUDDY_PERMISSIONS)) config.permissionsEnabled = true;

  if (options.quickCommandsEnabled != null) config.quickCommandsEnabled = !!options.quickCommandsEnabled;
  else if (truthy(env.WANGPET_HARDWARE_BUDDY_QUICK_COMMANDS) || truthy(env.WANGPET_QUICK_COMMANDS)) {
    config.quickCommandsEnabled = true;
  }
  if (truthy(env.WANGPET_HARDWARE_BUDDY_QUICK_COMMANDS_DISABLED) || truthy(env.WANGPET_QUICK_COMMANDS_DISABLED)) {
    config.quickCommandsEnabled = false;
  }

  if (env.WANGPET_HARDWARE_BUDDY_BACKEND) config.backend = env.WANGPET_HARDWARE_BUDDY_BACKEND;
  if (env.WANGPET_HARDWARE_BUDDY_ADDRESS) config.address = String(env.WANGPET_HARDWARE_BUDDY_ADDRESS).trim();
  if (env.WANGPET_HARDWARE_BUDDY_NAME_PREFIX) config.namePrefix = String(env.WANGPET_HARDWARE_BUDDY_NAME_PREFIX).trim();
  ...
  config.autoConnectByNamePrefix = !config.address && (hasSettings || !!env.WANGPET_HARDWARE_BUDDY_NAME_PREFIX);
  return config;
}
...
function buildSidecarArgs(options) {
  ...
  const backend = config.backend || env.WANGPET_HARDWARE_BUDDY_BACKEND || "bleak";
  const args = [
    defaultSidecarScript(coreRoot, env),
    "--backend",
    backend,
  ];
  if (env.WANGPET_HARDWARE_BUDDY_SCAN_TIMEOUT) {
    args.push("--scan-timeout", String(env.WANGPET_HARDWARE_BUDDY_SCAN_TIMEOUT));
  }
  if (env.WANGPET_HARDWARE_BUDDY_CONNECT_TIMEOUT) {
    args.push("--connect-timeout", String(env.WANGPET_HARDWARE_BUDDY_CONNECT_TIMEOUT));
  }
  if (config.namePrefix) {
    args.push("--name-prefix", config.namePrefix);
  }
  if (backend === "fake"
    && /^(true|false)$/i.test(String(env.WANGPET_HARDWARE_BUDDY_FAKE_SECURE || "").trim())) {
    args.push("--fake-secure", String(env.WANGPET_HARDWARE_BUDDY_FAKE_SECURE).trim().toLowerCase());
  }
  return args;
}
...
function classifyHardwareBuddyIssue(err) {
  ...
  if (code === "ENOENT" || lower.includes("spawn") && lower.includes("enoent")) {
    return {
      ...
      hint: "Install Python or configure WANGPET_HARDWARE_BUDDY_PYTHON.",
    };
  }
  ...
  if (code === "CORE_MISSING") {
    return {
      ...
      hint: "Clone https://github.com/rullerzhou-afk/clawstick next to this repo or set WANGPET_HARDWARE_BUDDY_ROOT.",
    };
  }
...
function createHardwareBuddyAdapter(options = {}) {
  ...
  const keepaliveMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_KEEPALIVE_MS, 10000);
  const autoConnectDelayMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_CONNECT_DELAY_MS, 1000);
  const autoConnectRetryMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_CONNECT_RETRY_MS, 15000);
  const autoConnectRetryMaxMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_CONNECT_RETRY_MAX_MS, 120000);
  const quickCommandBufferSize = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_QUICK_COMMAND_BUFFER_SIZE, 100);
  const quickCommandDedupeMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_QUICK_COMMAND_DEDUPE_MS, 30000);
  const notifyDebounceMs = options.notifyDebounceMs != null
    ? numberFromEnv(options.notifyDebounceMs, 50)
    : numberFromEnv(env.WANGPET_HARDWARE_BUDDY_NOTIFY_DEBOUNCE_MS, 50);
  const logThrottleMs = numberFromEnv(env.WANGPET_HARDWARE_BUDDY_LOG_THROTTLE_MS, 60000);
  ...
  function createSidecar(SidecarClient) {
    return new SidecarClient({
      command: options.command || env.WANGPET_HARDWARE_BUDDY_PYTHON || "python",

...
const SIDECAR_ENV_CONFIG = "WANGPET_BRIDGE_CONFIG";
const SIDECAR_ENV_TOKEN_FILE = "WANGPET_TG_BOT_TOKEN_FILE";
const SIDECAR_PATH_ENV = "WANGPET_CC_CONNECT_WANGPET_PATH";
const SIDECAR_BINARY_BASENAME = "cc-connect-wang-pet";
const SIDECAR_RESOURCE_ROOT = path.join("sidecars", "cc-connect-wang-pet");
...
// (which itself contains a line like `WANGPET_TG_BOT_TOKEN=<token>`). WangPet's
...
function resolveSidecarBinary(options = {}) {
  ...
  return {
    path: path.join(__dirname, "..", "bin", "cc-connect-wang-pet", sidecarPlatformArchDir({ platform, arch }), sidecarExecutableName(platform)),
    source: "dev",
  };
}
...
class TelegramApprovalSidecar extends EventEmitter {
  ...
  _handleStderr(chunk) {
    ...
    // can't blow up WangPet memory. Once the cap trips we flush what we have as

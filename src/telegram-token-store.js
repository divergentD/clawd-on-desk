...
//
// Plan §298-304 invariant: the WangPet source must NEVER read the bot-token
...
// WANGPET_TG_BOT_TOKEN_FILE). This module exposes a TelegramTokenStore interface
...
const TOKEN_LINE_RE = /^\s*WANGPET_TG_BOT_TOKEN\s*=\s*(.+?)\s*$/m;
...
function buildEnvFileText(token) {
  return `WANGPET_TG_BOT_TOKEN=${token}\n`;
}

#!/usr/bin/env bash
# WangPet Desktop Pet — Remote Hook Deployment
...
#   - WangPet running locally (for port detection)
...
if [ $# -lt 1 ]; then
  ...
  echo ""
  echo "Deploys WangPet hook files to a remote server so that"
  echo "Claude Code and Codex CLI states are synced back to your"
  echo "local WangPet via SSH reverse port forwarding."
  echo ""
  echo "Options:"
...
probe_login_shells() {
  ...
  do
    ...
    out="$("$shell" -lic 'printf "__WANGPET_REMOTE_NODE_PROBE__\n"; command -v node 2>/dev/null; which node 2>/dev/null; true' 2>/dev/null)"
    p="$(printf '%s\n' "$out" | awk 'found && $0 ~ /^\// { last=$0 } $0 == "__WANGPET_REMOTE_NODE_PROBE__" { found=1 } END { if (last) print last }')"
...
LOCAL_PORT=23333
RUNTIME_JSON="$HOME/.wang-pet/runtime.json"
...
echo "Deploying WangPet hooks to $SSH_TARGET..."
echo "  Local WangPet port: $LOCAL_PORT"
echo ""
...
REMOTE_NODE_BIN=$(printf '%s\n' "$REMOTE_NODE" | sed -n 's/^WANGPET_REMOTE_NODE_BIN=//p' | head -n 1)
REMOTE_NODE_VERSION=$(printf '%s\n' "$REMOTE_NODE" | sed -n 's/^WANGPET_REMOTE_NODE_VERSION=//p' | head -n 1)
REMOTE_NODE_SOURCE=$(printf '%s\n' "$REMOTE_NODE" | sed -n 's/^WANGPET_REMOTE_NODE_SOURCE=//p' | head -n 1)
...
if [ -n "$HOST_PREFIX" ]; then
  ...
  if printf '%s' "$HOST_PREFIX" | ssh "$SSH_TARGET" "cat > ~/.claude/hooks/wang-pet-host-prefix"; then
    echo "  [OK] Prefix written to ~/.claude/hooks/wang-pet-host-prefix"
  else
...
echo "back to your local WangPet through the SSH tunnel."

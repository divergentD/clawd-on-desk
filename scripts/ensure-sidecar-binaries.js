...
const SKIP_ENV = "WANGPET_SKIP_SIDECAR_FETCH";
const OVERRIDE_ENV = "WANGPET_CC_CONNECT_WANGPET_PATH";
...
function runtimeExecutableName(platform = process.platform) {
  return platform === "win32" ? "cc-connect-wang-pet.exe" : "cc-connect-wang-pet";
}
...
async function ensureCurrentPlatformSidecar(options = {}) {
  ...
  if (env[OVERRIDE_ENV]) {
    ...
    const launchMessage = options.strict
      ...
      : "WangPet will still launch. Fix the override path, or unset it to allow automatic sidecar fetch.";
    write(stderr, [
  ...
  write(stdout, `Missing ${target.dir} cc-connect-wang-pet sidecar; fetching pinned binary...\n`);
  try {

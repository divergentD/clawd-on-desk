...
Telegram Approval is an optional remote approval path for existing WangPet
permission bubbles. When a supported agent asks for tool permission, WangPet keeps
...
1. **Step 1 — Bot Token.** Create a dedicated bot with
   [@BotFather](https://t.me/botfather) using `/newbot`. Open WangPet Settings →
   **Remote Approval** → expand the **Telegram** card and paste the token into
   ...
   The token is stored outside `wang-pet-prefs.json` in WangPet's user-data
...
2. **Step 2 — Recipient.** Open [@userinfobot](https://t.me/userinfobot) in
   ...
   WangPet uses this one number both as the allowed approver (only this user can
...
- If the desktop bubble resolves first, WangPet aborts the in-flight Telegram
  approval request.
- Repeated Telegram taps after a request is already handled do not resolve the
  permission twice.
- Sidecar logs and WangPet logs redact Telegram tokens, chat ids, and token-like
  values.
...
WangPet logs as evidence.

1. Back up the current user-data `wang-pet-prefs.json`,
...
2. Seed the legacy upgrade path with `tgApproval.enabled=true`, a stored token,
   and a valid recipient. Start WangPet and verify Settings shows
...
3. Click **Test native and switch**. Verify the sidecar stops, Telegram receives
   ...
   Restart WangPet and verify it returns to `NATIVE_ACTIVE`.
...
Packaged builds ship the pinned `cc-connect-wang-pet` sidecar binary from
`bin/cc-connect-wang-pet/`. Source runs use the same directory layout, or the
`WANGPET_CC_CONNECT_WANGPET_PATH` override for development.

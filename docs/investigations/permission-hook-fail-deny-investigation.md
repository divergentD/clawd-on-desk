...
- **实际行为违反 CC 自己的文档** → 这是 CC bug，不是 WangPet 应该兜底的事
...
这违反 CC 自己的 hook 文档（详见下方"证据链"）。WangPet 无锅。

## 证据链

### 1. CC 给 Edit/Write 发 PermissionRequest（实证）

`%APPDATA%/wang-pet/permission-debug.log` 行 `2026-04-10T10:56:59.529Z`：
...
最初由鹿鹿 2026-04-10 发现：当时桌宠没开，Edit 调用一直被自动 deny，本以为是 WangPet 的 bug。诊断过程：
...
- 鹿鹿决定发 issue (#46193)，不在 WangPet 侧加 workaround
...
不要在 WangPet 侧加 hook fail-deny 的 workaround：

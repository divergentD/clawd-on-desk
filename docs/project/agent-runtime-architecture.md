...
Claude Code 状态同步（command hook，非阻塞）：
  Claude Code 触发事件
    → hooks/wang-pet-hook.js（零依赖 Node 脚本，stdin 读 JSON 取 session_id + source_pid）
...
Antigravity CLI (agy) 状态同步（hook-only，stdin JSON + stdout JSON）：
  ...
  Hook 注册到 ~/.gemini/config/hooks.json 的 WangPet hook group，**仅状态事件**。PreToolUse **故意不注册**，权限完全交给 agy 自己 5 选项 native menu（agy 1.0.1 LLM 主动调内置 ask_permission 工具触发，含 "Persist to settings.json" 持久规则）。Stop stdout 返回允许停止的 JSON。
...
Kiro CLI 状态同步（per-agent hook，stdin JSON）：
  ...
  custom agent 配置里，并额外维护一个 "wang-pet" agent（继承 kiro_default，启动时从 kiro_default
  重新同步以避免行为漂移）。内置 kiro_default 没有可编辑 JSON，用户需 `kiro-cli --agent WangPet`
  或 `/agent swap WangPet` 才能启用 hooks。
...
Kimi Code CLI（Kimi-CLI）状态同步（hook-only，config.toml）：
  ...
  Hook 注册到 ~/.kimi/config.toml 的 [[hooks]] 条目；WangPet 启动时会自动同步这些条目。
...
opencode 状态同步（in-process plugin，~0ms 延迟）：
  opencode 触发事件（session.created / session.status / message.part.updated 等）
    ...
    → translateEvent 映射（opencode v2 事件名 → PascalCase WangPet event 名）
...
Pi 状态同步（global extension，state-only）：
  Pi 触发 session_start / before_agent_start / tool_call / tool_result / agent_end 等事件
    → ~/.pi/agent/extensions/wang-pet/index.ts（Pi extension runtime）
    → hooks/pi-extension-core.js 映射为 PascalCase WangPet event 名
    → HTTP POST 127.0.0.1:23333/state
    → 同上状态机（agent_id: pi）
...
OpenClaw 状态同步（in-process plugin，state-only）：
  OpenClaw 触发 session_start / model_call_started / before_tool_call / after_tool_call / model_call_ended 等事件
    ...
    → 映射为 PascalCase WangPet event 名，POST body 只发送 allowlist 字段
    → fire-and-forget HTTP POST 127.0.0.1:23333/state
...
Hermes Agent 状态同步（Python plugin，Hermes SDK）：
  Hermes 触发 on_session_start / pre_llm_call / post_llm_call / pre_tool_call / post_tool_call / on_session_end / on_session_finalize / on_session_reset
    ...
    → 映射为 WangPet event + 同步 HTTP POST 127.0.0.1:23333/state
...
opencode 权限气泡（event hook + 反向 bridge，非阻塞）：
  opencode 请求权限 → event hook 收到 permission.asked
    → plugin POST /permission（带 bridge_url + bridge_token）→ WangPet 立即 200 ACK（不挂连接）
    → WangPet 创建 bubble 窗口 → 用户 Allow/Always/Deny
    → WangPet POST plugin 的反向 bridge → bridge 用 ctx.client._client.post() 调 opencode 内置 Hono 路由 /permission/:id/reply
    → opencode 执行对应行为（once/always/reject）

远程 SSH 状态同步（反向端口转发）：
  远程服务器上的 Claude Code / Codex CLI
    → hooks 通过 SSH 隧道 POST 到本地 127.0.0.1:23333
    → 同上状态机（WANGPET_REMOTE=1 模式跳过 PID 收集）
...
权限决策流（Codex official PermissionRequest command hook，阻塞）：
  Codex PermissionRequest
    ...
    → DND / disabled / bubble hidden / WangPet unavailable 时 stdout "{}"，Codex 回到原生审批提示
```
...
- Claude hook 同步时还会扫 `DEPRECATED_CORE_HOOKS`（当前含 `WorktreeCreate`）清掉旧版本留下的过时 WangPet hook 条目，仅删 command 指向 `wang-pet-hook.js` 的那条，用户自己写的同事件 hook 不动
...
- Hermes plugin 使用同步 POST，避免短命 `hermes -z` 进程退出前丢事件；WangPet 未启动时有短 cooldown，避免反复扫端口
- Hermes 的 `agent_pid` 当前是 plugin worker 进程 PID；`source_pid` 来自异步进程树解析，给终端聚焦使用
- Hermes config.yaml 是用户 YAML，不做 line-oriented 编辑；安装只复制托管 plugin 文件并调用 `hermes plugins enable WangPet`

## Pi Notes

- Pi 使用 global extension 目录 `~/.pi/agent/extensions/wang-pet`；安装器复制 `pi-extension.ts` 和自包含的 `pi-extension-core.js`
- Extension 运行目录不在 WangPet repo 内，不能依赖 `hooks/shared-process.js`；需要的进程树和 HTTP 逻辑保持在 extension 文件内
- 只在 `ctx.hasUI === true` 或交互式 TTY 模式上报状态，避免 print/RPC 模式污染桌宠状态
- Pi 是 state-only：`tool_call` 只上报 `PreToolUse` 状态，不等待 WangPet `/permission`，不弹权限气泡，也不调用 `ctx.ui.confirm()`
...
- 远程场景通过 `scripts/remote-deploy.sh` + SSH 反向端口转发，把远端 hook 事件回送到本地 WangPet
...
- 支持 en / zh / ko
- 文案集中在 `src/i18n.js`
- 语言偏好持久化到 `wang-pet-prefs.json`，启动时通过 `hydrate()` 灌入 controller

...
> Date: 2026-05-07
> Issue: https://github.com/rullerzhou-afk/wang-pet/issues/244
...
- WangPet version: 0.7.0, also reproduced on 0.6.2
- OS: macOS 15, Apple Silicon
- Low power idle: enabled

Initial report:

- Idle CPU was around 27-30% in one WangPet process and 18-20% in a second process.
- Combined idle CPU was around 45-50%, with 6-8% GPU.
- macOS showed WangPet as "Using Significant Energy".
...
2. Activity Monitor process attribution is not enough to prove that the second WangPet process is specifically main-process tick plus IPC. That is a plausible hypothesis, not a measured fact.
...
1. Enable Low power idle.
2. Leave WangPet in idle for at least 5 seconds.
...
6. Expected: WangPet resumes animation for that activity.
7. Return to idle.
...
- Compare built-in themes:
  - WangPet single-target SVG tracking

...
`D:\tmp\codex-pet-lab` has validated that Codex Pet style assets are practical for "wang-pet":
...
- rows already map cleanly to WangPet's major states
...
**A user sees a pet on a website, clicks one button, and the pet appears in WangPet.**

The implementation should keep WangPet's existing state machine, permissions, HUD, session dashboard, DND, and multi-agent integration. Codex Pet packages should become another asset source that WangPet can consume.

This plan is intentionally scoped to compatibility and import flow. It does not propose running OpenPet or any other external desktop pet runtime beside WangPet.
...
WangPet's `codex-pet-adapter` must carry its own local copy of the fixed atlas manifest. Do not infer frame counts or durations from `pet.json`; the minimal manifest does not include timing data today.
...
Current WangPet facts that affect this plan:
...
- Startup currently loads the selected theme immediately after `themeLoader.init()`. Codex Pet theme sync must run before the initial selected-theme load, or the startup fallback/self-heal path can replace a missing generated theme with built-in `wang-pet` and persist that fallback.
...
- `theme.json.source` is display/provenance metadata only. `.wang-pet-codex-pet.json` is the authoritative managed-theme marker and update/GC source of truth.

## Product Goal

Make Codex Pet packages feel native in WangPet while keeping the user mental model simple:

- Website language: "Open in WangPet" / "Take it for a walk"
- WangPet language: "Pets" or "Imported Pets"
- Technical language hidden from normal users: `theme.json`, atlas rows, spritesheet wrappers, state mappings

The user should not need to understand WangPet themes. A Codex Pet should feel like a character choice.

## Non-Goals

- Do not replace WangPet's current theme system.
- Do not run a second pet runtime or proxy state into OpenPet.
- Do not require pet authors to write WangPet `theme.json`.
...
3. WangPet scans `~/.codex/pets`.
4. WangPet shows Yoimiya under Settings -> Theme/Pets as an imported Codex Pet.
5. User selects it.
6. WangPet plays its atlas rows according to WangPet state.

This is the lowest-risk MVP because Petdex and compatible installers already establish the package in the expected directory. WangPet still applies its own stricter validation: Petdex currently accepts spritesheets smaller than the fixed Codex app atlas as long as they are valid submissions, while WangPet MVP only supports exact `1536x1872` 8x9 atlases.

### Flow 2: Website One-Click Import

1. User clicks `Open in WangPet` on the pet website.
2. Browser opens:

   ```text
   wang-pet://import-pet?url=https%3A%2F%2Fexample.test%2Fpets%2Fyoimiya%2Fyoimiya.zip
   ```

3. OS asks whether to open WangPet.
4. WangPet opens a pre-download import confirmation with the source host and package URL.
5. User clicks `Import and Use`.
6. WangPet downloads, validates, installs, materializes a wang-pet-compatible wrapper theme, and switches to the pet.
7. WangPet shows the pet display name after validation succeeds. MVP enforces strict download/package size caps instead of adding a second preflight dialog for display name and package size; a richer preflight can be revisited during polish.
...
2. User imports it in WangPet from Settings.
3. WangPet validates and installs it.
...
Some pet sites may not launch WangPet directly. Instead, they can give the user a prompt to paste into Codex, Claude Code, or another local agent:
...
This flow is lower-level than `Open in WangPet`, but it is valuable because it converges on the same canonical directory:
...
WangPet should treat this as a supported fallback acquisition path. If the agent installs the package correctly, WangPet's local scan and refresh should discover it without any wang-pet-specific import step.

## Recommended UX Copy

On the website:

- Primary button: `Open in WangPet`
...
```text
Works with Codex Pet compatible runtimes. Open it in WangPet, or install it to ~/.codex/pets.
```

If a website exposes an agent prompt, the prompt should avoid wang-pet-specific internals and should only promise installation into `~/.codex/pets`. WangPet can then say:

```text
After installing, open WangPet Settings -> Pets and click Refresh Imported Pets.
```
...
The Hatch Pet output path uses `spritesheet.webp`. Petdex validation also allows `spritesheet.png`, and some community sites distribute PNG atlases. WangPet should accept both `.webp` and `.png` when dimensions and transparency are valid. The generated wrappers should reference the actual `spritesheetPath` extension instead of assuming WebP.
...
`pet.json` currently does not contain row frame counts or frame durations. WangPet must ship this as a constant in `src/codex-pet-adapter.js`.
...
| Row | Codex Pet animation | WangPet use |
|---:|---|---|
...
- WangPet derives a filesystem/theme-safe `slug` separately. Prefer the package folder name if it is already lowercase ASCII letters, digits, or hyphens; otherwise slugify `id` or `displayName`.
- generated WangPet theme IDs use the safe slug, e.g. `codex-pet-yoimiya`, and preserve the original manifest `id` as `source.id`.
- slug collision rule:
...
- Petdex-installed packages that do not satisfy WangPet's exact atlas contract fail validation with a user-visible diagnostic instead of being treated as broken themes.
...
| WangPet logical state | Codex Pet row | Reason |
|---|---|---|
...
| WangPet reaction | Codex Pet row |
|---|---|
...
- Current WangPet menu code already hides/disables mini mode when `theme.miniMode.supported === false`; keep an explicit regression test for this.
...
- Codex Pet generated themes do not show WangPet's multi-session working tier progression. `working`, `juggling`, `sweeping`, and `carrying` all use `running`.
- Codex Pet generated themes do not provide native sleep/yawn/wake art. DND uses static or idle fallback visuals.
- Eye tracking is disabled because Codex Pet frames do not expose WangPet SVG DOM IDs.
...
2. Generate WangPet theme assets that wrap the spritesheet

The lowest-cost MVP is **generated wrapper theme**, but this is a gated decision, not an unproven assumption.

Why:

- WangPet already knows how to discover, load, select, preview, and persist themes.
- WangPet already routes states to filenames.
- WangPet already supports SVG animations through the `<object>` renderer path.
...
The adapter materializes a Codex Pet package into a normal WangPet user theme:
...
The adapter should generate a conservative WangPet theme:
...
- Keep `.wang-pet-codex-pet.json` as the authoritative managed-theme marker/update manifest. If `theme.json.source` and the marker disagree, marker data wins for refresh, update, and GC decisions.
- States must use WangPet's real schema: arrays of filenames or `{ "files": [...], "fallbackTo": "..." }`. Do not generate state objects like `{ "file": "...", "duration": 1200, "loop": true }`; those fail current validation.
...
- Adapter version mismatch must force full re-materialization of managed themes before theme load. Theme-loader does not need to read `.wang-pet-codex-pet.json`; the adapter owns wrapper regeneration and cache invalidation is then driven by changed wrapper/raster mtimes.
...
Goal: prove the generated-wrapper route under WangPet's real external theme path before building the adapter.

Status on 2026-05-05:

- Partial local spike has been run with `D:\tmp\codex-pet-lab\pets\pinky` and disposable user data under `D:\tmp\codex-pet-lab\preview\wang-pet-wrapper-spike\`.
...
- Legacy/default channel regression has been covered: missing `rendering` normalizes to `{ "svgChannel": "auto" }`, invalid values fail validation, ordinary external SVG themes still use the legacy non-object path, and built-in WangPet keeps its mixed channel behavior.
- Hidden Electron built-in WangPet matrix passed for `idle`, `working`, `sleeping`, `attention`, and `error`: `idle` remained `<object>`, the other tested states remained `<img>`, and every capture was nonblank.
...
- The full fixture exposed an important adapter rule: do not bind normal WangPet states directly to one-shot wrappers. State bindings should use loop/static wrappers; one-shot wrappers belong to reactions, where the renderer deliberately swaps a fresh object document.
...
- keep spike files outside the repo, e.g. `D:\tmp\codex-pet-lab\preview\wang-pet-wrapper-spike-theme\`
...
- switch back from the Codex Pet spike theme to built-in `wang-pet` and confirm eye tracking attaches again
...
- materialize a managed WangPet theme folder
...
This lets WangPet update generated wrappers safely without touching user-authored themes.

Management boundary:

- `.wang-pet-codex-pet.json` is authoritative for refresh, update, and garbage collection decisions.
...
2. WangPet calls `syncCodexPetThemes()`.
...
Implementation constraint: in the current app startup path, step 2 must happen before the first `themeLoader.loadTheme()` for the stored selected theme. If async startup sync is used instead, the theme fallback/self-heal logic must be changed so a temporarily missing managed Codex Pet theme does not silently rewrite the user's selected theme to `wang-pet`.
...
- If the source package disappears while the generated theme is active, switch to built-in `wang-pet`, persist that fallback explicitly, then remove the managed generated theme.
...
- if `npx petdex install yoimiya` has installed a package, WangPet can use it without manual conversion
...
```text
wang-pet://import-pet?url=https%3A%2F%2Fsite.test%2Fpets%2Fyoimiya%2Fyoimiya.zip
wang-pet://import-pet?url=https%3A%2F%2Fsite.test%2Fpets%2Fyoimiya%2Fpet.json
```
...
- Windows/Linux:
  ...
  - packaged app registers protocol with `app.setAsDefaultProtocolClient("wang-pet")`
  - Windows dev builds must pass explicit executable and argv. Under the current Electron dev launch, prefer `app.setAsDefaultProtocolClient("wang-pet", process.execPath, [path.resolve(process.argv[1] || ".")])` after verifying the actual `process.argv` printed by `npm start`.
- macOS:
  - handle `open-url`
...
- WangPet owns the protocol handler, local confirmation UI, validation, install, and theme switch.
- Petdex or any other pet gallery owns adding its own `Open in WangPet` button. Treat that as an upstream PR / collaboration item, not something the WangPet codebase alone can ship.
- Phase 3 can be accepted locally with a demo HTML page or README link that emits `wang-pet://import-pet?...`; public gallery support is a separate integration milestone.

Packaging check:

- Add protocol metadata to electron-builder config where required by macOS and packaged builds.
- On Windows, verify whether the NSIS install mode registers the handler per-user or per-machine for WangPet's current installer settings, whether elevation changes that scope, and whether updates preserve the registration.
...
- primary `Open in WangPet` button emits `wang-pet://import-pet?...`
...
- WangPet state `working` resolves to `codex-pet-running-loop.svg`
- WangPet state `notification` resolves to `codex-pet-waiting-loop.svg`
- WangPet state `error` resolves to `codex-pet-failed-loop.svg`
...
- switching from Codex Pet back to built-in `wang-pet` re-attaches eye tracking normally
...
- start WangPet with packages already in `~/.codex/pets`
...
- uninstall/delete package and verify WangPet degrades gracefully
- protocol import with WangPet already running
- protocol import with WangPet closed
- agent-assisted install prompt flow: unzip a package into `~/.codex/pets`, then refresh WangPet imported pets
...
- treat agent-assisted prompts as a fallback, not the safest path. The prompt should include zip-slip checks, but damage can occur before WangPet's later scan. Prefer `wang-pet://` import or Petdex for normal users.

Rights:

- imported pets may be fan art or third-party character art
- WangPet should not imply the project owns or endorses imported art
...
- website/package metadata can carry source and license fields as optional extensions, but WangPet must not require them

Suggested disclaimer in import dialog:

```text
Only import pets you have the right to use. WangPet will store a local copy and use it as your desktop pet.
```

## Migration And Compatibility

Existing WangPet users:
...
- no need to reinstall through WangPet
...
1. Should WangPet use `~/.codex/pets` as the canonical install location for one-click imports, or copy into WangPet userData and optionally mirror to `~/.codex/pets`?
...
3. Should import be one-click with no confirmation?
   ...
   Recommendation: no for MVP. Use one click on website plus one confirmation in WangPet. Remove confirmation only for trusted sources later.

4. Should WangPet support website scraping?
...
6. Should website-generated agent prompts be treated as an official install path?
   ...
   Recommendation: yes, but only as a fallback. The prompt should install into `~/.codex/pets`; WangPet should not rely on the prompt wording for trust or validation. WangPet still validates packages when it scans them.
...
- a pet installed to `~/.codex/pets/<id>` appears in WangPet without manual conversion
- the user can select it from Settings
- WangPet states visibly animate using the correct atlas rows
...
- a website button can open WangPet through `wang-pet://import-pet`
- WangPet imports, validates, and switches to the pet
...
7. Update website button to emit `wang-pet://import-pet` and keep Petdex/zip/agent prompt fallbacks.

...
test("settings-tab-remote-ssh.js loads in a sandbox via the same IIFE pattern as siblings", () => {
  ...
  // IIFE registration check — must call wangpetSettingsTabRemoteSsh = { init }.
  assert.match(code, /root\.wangpetSettingsTabRemoteSsh\s*=\s*\{\s*init\s*\}/);
...
test("settings-tab-remote-ssh.js can be evaluated without DOM (no top-level DOM access)", () => {
  ...
  // to register `wangpetSettingsTabRemoteSsh`; render() is what actually touches
  ...
  assert.ok(sandbox.globalThis.wangpetSettingsTabRemoteSsh, "tab module must register on globalThis");
  assert.equal(typeof sandbox.globalThis.wangpetSettingsTabRemoteSsh.init, "function");
});

...
function makeTempHome() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-server-config-"));
...
describe("server-config helpers", () => {
  it("clearRuntimeConfig removes runtime.json when present", () => {
    ...
    const runtimeDir = path.join(tmpHome, ".wang-pet");
    fs.mkdirSync(runtimeDir, { recursive: true });
    const runtimePath = path.join(runtimeDir, "runtime.json");
    fs.writeFileSync(runtimePath, JSON.stringify({ app: "wang-pet", port: 23333 }));
  ...
  it("probePort recognizes signed WangPet responses", async () => {
    await new Promise((resolve, reject) => {
      ...
      serverConfig.probePort(23337, 100, (ok) => {
        ...
        httpGet(_options, onResponse) {
          const res = {
            headers: { "x-wang-pet-server": "wang-pet" },
            setEncoding() {},
            on(event, handler) {
  ...
  describe("resolveNodeBin on Windows", () => {
    ...
    it("rejects the packaged WangPet Electron host as execPath", () => {
    ...
    it("rejects WangPet.exe even when accessSync says it exists", () => {
      // accessSync only succeeds for the WangPet.exe path so validator rejection
      // is the only thing standing between us and a wrong return value.
      const wangpetExe = "C:\\Program Files\\wang-pet\\wang-pet.exe";
      const result = serverConfig.resolveNodeBin({
        platform: "win32",
        env: WIN_ENV,
        execPath: wangpetExe,
        accessSync(candidate) {
          if (candidate === wangpetExe) return;
          throw new Error("ENOENT");
        },
        execFileSync() { return ""; },
      });
    ...
    it("validateWindowsNodeCandidate rejects WangPet, Electron, scoop shims, and non-node basenames", () => {
      const v = serverConfig.validateWindowsNodeCandidate;
      assert.strictEqual(v("C:\\Program Files\\nodejs\\node.exe"), "C:\\Program Files\\nodejs\\node.exe");
      assert.strictEqual(v("C:\\Program Files\\wang-pet\\wang-pet.exe"), null);
  ...
  it("postStateToRunningServer raises short timeouts in WANGPET_REMOTE mode", async () => {
    const timeouts = [];
  ...
  it("postStateToRunningServer lets remote false override WANGPET_REMOTE env", async () => {
    const timeouts = [];
  ...
  it("postPermissionToRunningServer raises discovery timeout in WANGPET_REMOTE mode", async () => {
    ...
    await new Promise((resolve, reject) => {
      serverConfig.postPermissionToRunningServer(
        JSON.stringify({ tool_name: "bash" }),
        {
          probeTimeoutMs: 100,
          env: { WANGPET_REMOTE: "1" },
          discoverwangpetPort(options, cb) {

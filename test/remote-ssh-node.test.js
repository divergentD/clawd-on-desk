...
test("parseRemoteNodeProbeOutput extracts markers while ignoring shell noise", () => {
  const parsed = parseRemoteNodeProbeOutput([
    "oh-my-zsh banner",
    "WANGPET_REMOTE_NODE_BIN=/Users/u/.nvm/versions/node/v20.10.0/bin/node",
    "WANGPET_REMOTE_NODE_VERSION=v20.10.0",
    "WANGPET_REMOTE_NODE_SOURCE=shell:/bin/zsh",
  ].join("\n"));
...
test("parseRemoteNodeProbeOutput rejects bare node and invalid versions", () => {
  assert.equal(parseRemoteNodeProbeOutput("WANGPET_REMOTE_NODE_BIN=node\nWANGPET_REMOTE_NODE_VERSION=v20\n"), null);
  assert.equal(parseRemoteNodeProbeOutput("WANGPET_REMOTE_NODE_BIN=/usr/bin/node\nWANGPET_REMOTE_NODE_VERSION=20\n"), null);
  assert.equal(parseRemoteNodeProbeOutput("WANGPET_REMOTE_NODE_BIN=/usr/bin/node\nWANGPET_REMOTE_NODE_VERSION=v12.22.12\n"), null);
});
...
test("resolveRemoteNodeBin records ssh command and caches successful result", async () => {
  ...
  const { spawn, calls } = makeRecordingSpawn([{
    stdout: [
      "WANGPET_REMOTE_NODE_BIN=/home/me/.nvm/versions/node/v22.1.0/bin/node",
      "WANGPET_REMOTE_NODE_VERSION=v22.1.0",
      "WANGPET_REMOTE_NODE_SOURCE=shell:/bin/bash",
    ].join("\n"),
  }]);
...
test("resolveRemoteNodeBin can verify a cached path and fall back when stale", async () => {
  ...
  const { spawn, calls } = makeRecordingSpawn([
    ...
    {
      stdout: [
        "WANGPET_REMOTE_NODE_BIN=/usr/local/bin/node",
        "WANGPET_REMOTE_NODE_VERSION=v22.1.0",
        "WANGPET_REMOTE_NODE_SOURCE=path",
      ].join("\n"),
    },
  ]);

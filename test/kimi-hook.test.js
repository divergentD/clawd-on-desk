...
describe("Kimi hook script", () => {
  ...
  it("defaults to explicit-only (no suspect) for permission tools without explicit signal", () => {
    const oldDisable = process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION;
    const oldImmediate = process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE;
    const oldSuspect = process.env.WANGPET_KIMI_PERMISSION_SUSPECT;
    try {
      delete process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION;
      delete process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE;
      delete process.env.WANGPET_KIMI_PERMISSION_SUSPECT;
      const resolve = () => ({
        stablePid: 12345,
        agentPid: null,
        detectedEditor: null,
      ...
      if (oldDisable == null) delete process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION;
      else process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION = oldDisable;
      if (oldImmediate == null) delete process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE;
      else process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE = oldImmediate;
      if (oldSuspect == null) delete process.env.WANGPET_KIMI_PERMISSION_SUSPECT;
      else process.env.WANGPET_KIMI_PERMISSION_SUSPECT = oldSuspect;
    }
  });

  it("WANGPET_KIMI_PERMISSION_SUSPECT=1 enables deferred suspect mode", () => {
    const oldSuspect = process.env.WANGPET_KIMI_PERMISSION_SUSPECT;
    try {
      process.env.WANGPET_KIMI_PERMISSION_SUSPECT = "1";
      const resolve = () => ({
        stablePid: 12345,
        agentPid: null,
        detectedEditor: null,
  ...
  it("WANGPET_KIMI_PERMISSION_MODE controls default classification persistently", () => {
    const oldMode = process.env.WANGPET_KIMI_PERMISSION_MODE;
    try {
      process.env.WANGPET_KIMI_PERMISSION_MODE = MODE_SUSPECT;
      ...
      process.env.WANGPET_KIMI_PERMISSION_MODE = MODE_EXPLICIT;
      ...
      if (oldMode == null) delete process.env.WANGPET_KIMI_PERMISSION_MODE;
      else process.env.WANGPET_KIMI_PERMISSION_MODE = oldMode;
    }
  });

  it("WANGPET_KIMI_PERMISSION_IMMEDIATE=1 restores legacy instant notification mapping", () => {
    const oldImmediate = process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE;
    try {
      process.env.WANGPET_KIMI_PERMISSION_IMMEDIATE = "1";
      const resolve = () => ({
        stablePid: 12345,
        agentPid: null,
        detectedEditor: null,
  ...
  it("keeps PreToolUse as working without permission_suspect when disable env is set", () => {
    const old = process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION;
    try {
      process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION = "1";
      const resolve = () => ({
        stablePid: 12345,
        agentPid: null,
        detectedEditor: null,
      ...
      if (old == null) delete process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION;
      else process.env.WANGPET_KIMI_DISABLE_PRETOOL_PERMISSION = old;
    }
  });
  ...
  it("reads latest token usage from Kimi wire.jsonl for the session", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-kimi-wire-"));
    try {
  ...
  it("reads Kimi wire token usage when session id already has agent prefix", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-kimi-wire-prefix-"));
    try {
      const sessionId = "8f605215-fdb7-4857-b07e-a213d3a3ab7d";
      const sessionDir = path.join(tmpDir, "workspace-hash", sessionId);
      fs.mkdirSync(sessionDir, { recursive: true });
  ...
  it("allows overriding permission tools through env parser", () => {
    const old = process.env.WANGPET_KIMI_PERMISSION_TOOLS;
    try {
      delete process.env.WANGPET_KIMI_PERMISSION_TOOLS;
      assert.deepStrictEqual([...resolvePermissionTools()], DEFAULT_PERMISSION_TOOLS);

      process.env.WANGPET_KIMI_PERMISSION_TOOLS = "shell,ask_user_question";
      assert.deepStrictEqual([...resolvePermissionTools()], ["shell", "askuserquestion"]);
    } finally {
      if (old == null) delete process.env.WANGPET_KIMI_PERMISSION_TOOLS;
      else process.env.WANGPET_KIMI_PERMISSION_TOOLS = old;
    }
  });
  ...
  it("uses default debug log size cap when env is unset/invalid", () => {
    const old = process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES;
    try {
      delete process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES;
      assert.strictEqual(readHookDebugMaxBytes(), DEFAULT_HOOK_DEBUG_MAX_BYTES);

      process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES = "not-a-number";
      assert.strictEqual(readHookDebugMaxBytes(), DEFAULT_HOOK_DEBUG_MAX_BYTES);
    } finally {
      if (old == null) delete process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES;
      else process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES = old;
    }
  });

  it("stops writing debug log when file reaches max bytes cap", () => {
    const oldDebug = process.env.WANGPET_KIMI_HOOK_DEBUG;
    const oldPath = process.env.WANGPET_KIMI_HOOK_DEBUG_PATH;
    const oldMax = process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES;
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-kimi-hook-"));
    const debugFile = path.join(tmpDir, "kimi-hook-debug.jsonl");
    try {
      process.env.WANGPET_KIMI_HOOK_DEBUG = "1";
      process.env.WANGPET_KIMI_HOOK_DEBUG_PATH = debugFile;
      process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES = "30";
      ...
      fs.rmSync(tmpDir, { recursive: true, force: true });
      if (oldDebug == null) delete process.env.WANGPET_KIMI_HOOK_DEBUG;
      else process.env.WANGPET_KIMI_HOOK_DEBUG = oldDebug;
      if (oldPath == null) delete process.env.WANGPET_KIMI_HOOK_DEBUG_PATH;
      else process.env.WANGPET_KIMI_HOOK_DEBUG_PATH = oldPath;
      if (oldMax == null) delete process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES;
      else process.env.WANGPET_KIMI_HOOK_DEBUG_MAX_BYTES = oldMax;
    }
  });
});

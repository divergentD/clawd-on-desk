...
function nodeProbeStdout(nodeBin = "/usr/bin/node", version = "v20.10.0", source = "path") {
  return [
    `WANGPET_REMOTE_NODE_BIN=${nodeBin}`,
    `WANGPET_REMOTE_NODE_VERSION=${version}`,
    `WANGPET_REMOTE_NODE_SOURCE=${source}`,
  ].join("\n");
}
...
test("deploy: with hostPrefix triggers host-prefix step via ssh stdin", async () => {
  ...
  // The 4th call (index 3) must be the host-prefix ssh: cat > ~/.claude/hooks/wang-pet-host-prefix
  const hpCall = calls[3];
  assert.equal(hpCall.command, "ssh");
  const remoteCmd = hpCall.args[hpCall.args.length - 1];
  assert.equal(remoteCmd, "cat > ~/.claude/hooks/wang-pet-host-prefix");
...
test("startCodexMonitor pre-cleans then launches new monitor", async () => {
  ...
  assert.match(cleanCmd, /\.wang-pet-codex-monitor\.pid/);
  ...
  assert.match(startCmd, /echo \$! > ~\/\.wang-pet-codex-monitor\.pid/);
});

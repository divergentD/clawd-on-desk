...
function makeFixture(settingsText, options = {}) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-uninstall-"));
  const home = path.join(root, "User With Space 中文");
  const claudeDir = path.join(home, ".claude");
  const installDir = path.join(root, "WangPet Install Dir");
  fs.mkdirSync(claudeDir, { recursive: true });
  fs.mkdirSync(installDir, { recursive: true });

  const markerPath = path.join(installDir, ".wang-pet-install-user-home");
...
describe("Windows NSIS Claude hook uninstall cleanup", () => {
  ...
  it("keeps the PowerShell permission port list in sync with SERVER_PORTS", () => {
    ...
    const match = script.match(/\$wangpetPermissionPorts\s*=\s*@\(([^)]*)\)/);
  ...
  it("removes only WangPet hooks while preserving third-party hooks and array shape", {
    ...
    const fixture = makeFixture(JSON.stringify({
      ...
      hooks: {
        SessionStart: [
          {
            matcher: "",
            hooks: [{ type: "command", command: '& "node" "C:\\wang-pet\\hooks\\auto-start.js"' }],
          },
          {
            matcher: "",
        ...
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", command: '& "node" "C:\\wang-pet\\hooks\\wang-pet-hook.js" Stop' }],
          },
        ],
        PreToolUse: [
          {
            ...
            hooks: [
              ...
              { type: "command", command: '& "node" "C:\\wang-pet\\hooks\\wang-pet-hook.js" PreToolUse' },
            ],
          },
        ],
        PermissionRequest: [
    ...
    try {
      ...
      const backups = fs.readdirSync(fixture.claudeDir).filter((name) => /^settings\.json\.wang-pet-uninstall-.*\.bak$/.test(name));
  ...
  it("keeps command marker matching case-sensitive to match JS cleanup semantics", {
    ...
    const fixture = makeFixture(JSON.stringify({
      hooks: {
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", command: '& "node" "C:\\User\\wang-pet-Hook.js" Stop' }],
          },
        ],
      },
    }, null, 2));
    ...
    try {
      runCleanup(fixture);
      const settings = readJson(fixture.settingsPath);
      assert.strictEqual(settings.hooks.Stop[0].hooks[0].command, '& "node" "C:\\User\\wang-pet-Hook.js" Stop');
    } finally {
      fixture.cleanup();
    }
  });
  ...
  it("leaves malformed settings untouched", {
    ...
    try {
      ...
      const backups = fs.readdirSync(fixture.claudeDir).filter((name) => name.includes("wang-pet-uninstall"));

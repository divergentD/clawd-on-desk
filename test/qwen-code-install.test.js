...
function makeTempSettingsFile(initial = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-qwen-"));
...
describe("Qwen Code hook installer", () => {
  it("registers the Phase 1 events with event-specific matcher and timeout", () => {
    ...
    for (const event of QWEN_CODE_HOOK_EVENTS) {
      ...
      assert.strictEqual(entry.hooks[0].name, "wang-pet");
  ...
  it("splits WangPet out of shared matcher entries", () => {
    const settingsPath = makeTempSettingsFile({
      hooks: {
        PreToolUse: [{
          matcher: "Bash",
          hooks: [
            ...
            { type: "command", command: '"/old/node" "/old/path/qwen-code-hook.js" "PreToolUse"', name: "wang-pet" },
          ],
        }],
      },
    });
  ...
  it("skips startup auto-sync when ~/.qwen does not exist", () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-qwen-home-"));
    tempDirs.push(tmpDir);
  ...
  it("wraps Windows commands in PowerShell -EncodedCommand to bypass cmd /s quote stripping", () => {
    ...
    const command = buildQwenCodeHookCommand(
      nodeBin,
      "D:/wang-pet/hooks/qwen-code-hook.js",
      "PermissionRequest",
      {
        platform: "win32",
    ...
    assert.strictEqual(
      ...
      `& '${nodeBin}' 'D:/wang-pet/hooks/qwen-code-hook.js' 'PermissionRequest'`
    );
  });
  ...
  it("unregister removes encoded WangPet commands while preserving user hooks", () => {
    const settingsPath = makeTempSettingsFile({
      hooks: {
        PreToolUse: [{
          matcher: "*",

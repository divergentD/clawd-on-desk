...
describe("loadTheme level option", () => {
  ...
  it("uses existing visibly distinct wang-pet skins for levels 2 through 4", () => {
    themeLoader.init(path.join(__dirname, "..", "src"));

    const lvl2 = themeLoader.loadTheme("wang-pet", { strict: true, level: 2 });
    assert.strictEqual(lvl2._levelId, 2);
    assert.deepStrictEqual(lvl2.states.idle, ["wang-pet-working-typing-boss.svg"]);
    assert.deepStrictEqual(lvl2.states.working, ["wang-pet-working-typing-boss.svg"]);
    assert.deepStrictEqual(lvl2.workingTiers, [{ minSessions: 1, file: "wang-pet-working-typing-boss.svg" }]);

    const lvl3 = themeLoader.loadTheme("wang-pet", { strict: true, level: 3 });
    assert.strictEqual(lvl3._levelId, 3);
    assert.deepStrictEqual(lvl3.states.idle, ["wang-pet-working-wizard.svg"]);
    assert.deepStrictEqual(lvl3.states.working, ["wang-pet-working-wizard.svg"]);
    assert.deepStrictEqual(lvl3.workingTiers, [{ minSessions: 1, file: "wang-pet-working-wizard.svg" }]);

    const lvl4 = themeLoader.loadTheme("wang-pet", { strict: true, level: 4 });
    assert.strictEqual(lvl4._levelId, 4);
    assert.deepStrictEqual(lvl4.states.idle, ["wang-pet-working-ultrathink.svg"]);
    assert.deepStrictEqual(lvl4.states.working, ["wang-pet-working-ultrathink.svg"]);
    assert.deepStrictEqual(lvl4.workingTiers, [{ minSessions: 1, file: "wang-pet-working-ultrathink.svg" }]);
  });

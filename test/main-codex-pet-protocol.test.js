...
test("main wires wang-pet:// protocol dispatch through the Codex Pet importer", () => {
  ...
  assert.ok(runtimeSource.includes("codexPetImporter.parsewangpetImportUrl(rawUrl)"));
...
test("package metadata registers the WangPet protocol and exposes dev registration", () => {
  ...
  assert.ok(protocols.some((entry) => Array.isArray(entry.schemes) && entry.schemes.includes("wang-pet")));

...
test("sidecarBinaryPath uses resolver-compatible binary names", () => {
  assert.equal(
    ...
    path.join("D:\\repo", "bin", "cc-connect-wang-pet", "windows-arm64", "cc-connect-wang-pet.exe")
  );
  assert.equal(
    sidecarBinaryPath("/repo", "linux", "x64"),
    path.join("/repo", "bin", "cc-connect-wang-pet", "linux-x64", "cc-connect-wang-pet")
  );
});
  ...
  assert.deepEqual(result.missing, [
    {
      platform: "windows",
      arch: "arm64",
      path: path.join(rootDir, "bin", "cc-connect-wang-pet", "windows-arm64", "cc-connect-wang-pet.exe"),
    },
  ]);
});

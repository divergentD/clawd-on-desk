...
describe("doctor report redaction", () => {
  ...
  it("redacts configured WangPet app roots without consuming the rest of the path", () => {
    const out = redact([
  ...
  it("redacts nested doctor results before they are sent to the Settings UI", () => {
    const result = redactDoctorResult({
      checks: [{
        ...
        detail: "C:\\Users\\Alice\\.wang-pet\\runtime.json",
        details: [{
...
describe("formatDiagnosticReport", () => {
  it("formats structured agent diagnostics into visible detail text", () => {
    const detail = formatAgentDetail({
      ...
      codexHookTrust: {
        ...
        value: "needs-review",
        detail: "2/2 WangPet Codex hook(s) need Codex /hooks review: PermissionRequest, Stop",
      },
      kiroScan: {
        fullyValidFiles: ["wang-pet.json"],
    ...
    assert.match(detail, /valid=WangPet\.json/);
  ...
  it("formats Gemini supplementary diagnostics into visible detail text", () => {
    const detail = formatAgentDetail({
      detail: "Gemini hooks are disabled in settings.json; WangPet preserves this user setting and will not receive hook events",
      supplementary: {
        key: "gemini_hooks",
  ...
  it("formats summary and agent integration details", () => {
    ...
    assert.match(report, /# WangPet Diagnostic Report/);
    ...
    assert.match(report, /valid=WangPet\.json/);
  ...
  it("keeps the fallback file table when no HTTP event reached WangPet", () => {
    const report = formatDiagnosticReport({
      generatedAt: "2026-04-28T14:32:00.000Z",
      overall: { status: "warning", issueCount: 1 },
      checks: [],
      connectionTest: {
        status: "http-blocked",
        level: "warning",
        detail: "File activity changed, but no HTTP hook event reached WangPet.",
        events: [],
        fileActivity: [{
          agentId: "codex",
          source: "file-mtime",

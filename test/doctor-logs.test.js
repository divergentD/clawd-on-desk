...
describe("doctor log opener", () => {
  ...
  it("resolves the newest allowed log across ~/.wang-pet and userData", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-doctor-logs-"));
    const homeDir = path.join(tmp, "home");
    const userDataDir = path.join(tmp, "userData");
    const wangpetDir = path.join(homeDir, ".wang-pet");
    fs.mkdirSync(wangpetDir, { recursive: true });
    fs.mkdirSync(userDataDir, { recursive: true });
    const oldLog = path.join(wangpetDir, "gemini-debug.log");
    ...
    const target = resolvewangpetLogTarget({ homeDir, userDataDir });
  ...
  it("includes focus-debug.log in default log discovery", () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-doctor-focus-log-"));
    const userDataDir = path.join(tmp, "userData");
    fs.mkdirSync(userDataDir, { recursive: true });
    const focusLog = path.join(userDataDir, "focus-debug.log");
    fs.writeFileSync(focusLog, "focus");

    const target = resolvewangpetLogTarget({ homeDir: tmp, userDataDir });
  ...
  it("rejects path traversal requests", () => {
    const target = resolvewangpetLogTarget({
  ...
  it("opens the fallback ~/.wang-pet directory when no log exists", async () => {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-doctor-log-open-"));
    const opened = [];
    const result = await openwangpetLog({
      homeDir: tmp,
      shell: {
    ...
    assert.strictEqual(opened[0], path.join(tmp, ".wang-pet"));

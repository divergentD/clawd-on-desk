...
describe("hardware buddy adapter", () => {
  it("is disabled unless explicitly requested by env", () => {
    ...
    assert.strictEqual(isEnabledFromEnv({ WANGPET_HARDWARE_BUDDY: "1" }), true);
    assert.strictEqual(isEnabledFromEnv({ WANGPET_HARDWARE_BUDDY_BACKEND: "bleak" }), true);
    assert.strictEqual(isEnabledFromEnv({ WANGPET_HARDWARE_BUDDY_ADDRESS: "AA:BB" }), true);
    assert.strictEqual(isEnabledFromEnv({
      WANGPET_HARDWARE_BUDDY: "1",
      WANGPET_HARDWARE_BUDDY_DISABLED: "true",
    }), false);
  });
  ...
  it("starts state-only controller and suppresses pending permissions by default", () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: { WANGPET_HARDWARE_BUDDY: "1", WANGPET_HARDWARE_BUDDY_BACKEND: "fake" },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,
        SidecarClient: FakeSidecarClient,
      },
  ...
  it("cleans up partial startup state when sidecar start throws", () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: {
        WANGPET_HARDWARE_BUDDY: "1",
        WANGPET_HARDWARE_BUDDY_CONNECT_RETRY_MS: "25",
      },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,
  ...
  it("auto-connects when an address is configured", async () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: {
        WANGPET_HARDWARE_BUDDY: "1",
        WANGPET_HARDWARE_BUDDY_BACKEND: "fake",
        WANGPET_HARDWARE_BUDDY_ADDRESS: "FAKE:CLAWSTICK",
        WANGPET_HARDWARE_BUDDY_CONNECT_DELAY_MS: "0",
      },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,
        SidecarClient: FakeSidecarClient,
  ...
  it("retries auto-connect after a connection error", () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: {
        ...
        WANGPET_HARDWARE_BUDDY_BACKEND: "fake",
        WANGPET_HARDWARE_BUDDY_ADDRESS: "FAKE:CLAWSTICK",
        WANGPET_HARDWARE_BUDDY_CONNECT_DELAY_MS: "10",
        WANGPET_HARDWARE_BUDDY_CONNECT_RETRY_MS: "25",
      },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,
  ...
  it("passes the fake secure setting through to the sidecar", () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: {
        WANGPET_HARDWARE_BUDDY: "1",
        WANGPET_HARDWARE_BUDDY_BACKEND: "fake",
        WANGPET_HARDWARE_BUDDY_FAKE_SECURE: "false",
      },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,
        SidecarClient: FakeSidecarClient,
  ...
  it("ignores the fake secure setting for non-fake backends", () => {
    ...
    const adapter = createHardwareBuddyAdapter({
      env: {
        WANGPET_HARDWARE_BUDDY: "1",
        WANGPET_HARDWARE_BUDDY_BACKEND: "bleak",
        WANGPET_HARDWARE_BUDDY_FAKE_SECURE: "false",
      },
      coreModules: {
        HardwareBuddyController: FakeHardwareBuddyController,

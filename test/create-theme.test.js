...
function makeTmpDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-create-theme-"));
...
describe("create-theme defaults", () => {
  it("resolves the platform-specific user themes directory", () => {
    assert.strictEqual(
      ...
      path.win32.join("C:\\Users\\Ruller\\AppData\\Roaming", "wang-pet", "themes")
    );
    assert.strictEqual(
      createTheme.getDefaultThemesRoot("darwin", {}, "/Users/ruller"),
      "/Users/ruller/Library/Application Support/wang-pet/themes"
    );
    assert.strictEqual(
      createTheme.getDefaultThemesRoot("linux", { XDG_CONFIG_HOME: "/tmp/config-home" }, "/home/ruller"),
      "/tmp/config-home/wang-pet/themes"
    );
  });

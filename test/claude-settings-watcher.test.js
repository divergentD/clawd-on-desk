...
function makeWatcher(overrides = {}) {
  ...
  const watcher = createClaudeSettingsWatcher({
    ...
    isAgentEnabled: () => true,
    syncwangpetHooks: () => syncCalls.push("claude"),
    ...ctxOverrides,
  });
...
describe("settingsNeedClaudeHookResync", () => {
  ...
  it("requires both managed command hooks and the expected PermissionRequest URL", () => {
    ...
    const intact = JSON.stringify({
      hooks: {
        Stop: [{ matcher: "", hooks: [{ type: "command", command: "node wang-pet-hook.js Stop" }] }],
...
describe("createClaudeSettingsWatcher", () => {
  ...
  it("re-syncs when a healthy wang-pet-only baseline loses hooks", () => {
    const { watcher, timers, syncCalls, getWatcher, setSettingsRaw } = makeWatcher();
  ...
  it("treats WangPet auto-start hooks as managed when checking for suspicious shrink", () => {
    const wangpetOnlyWithAutoStart = JSON.stringify({
      hooks: {
        SessionStart: [{
          matcher: "",
          hooks: [
            { type: "command", command: 'node "/tmp/auto-start.js"' },
            { type: "command", command: 'node "/tmp/wang-pet-hook.js" SessionStart' },
          ],
        }],
        PermissionRequest: [{
          matcher: "",
    ...
    const { watcher, timers, syncCalls, getWatcher, setSettingsRaw } = makeWatcher({
      initialSettingsRaw: wangpetOnlyWithAutoStart,
    });
...
describe("createClaudeSettingsWatcher — suspicious shrink protection", () => {
  ...
  const HEALTHY_SETTINGS = JSON.stringify({
    ...
    hooks: {
      Stop: [{
        matcher: "",
        hooks: [
          { type: "command", command: 'node "/tmp/wang-pet-hook.js" Stop' },
  ...
  // Marker-removed minor shrink — user removes WangPet hook only; one command hook lost (13 -> 12), keys unchanged.
  ...
  const SETTINGS_AFTER_USER_REMOVES_WANGPET_AND_ONE_THIRD_PARTY_HOOK = JSON.stringify({
    env: { FOO: "bar" },
    permissions: { allow: ["*"], deny: [], defaultMode: "ask" },
    enabledPlugins: { a: 1, b: 2 },
    skillOverrides: { sk1: true },
  ...
  it("allows resync on first tick when start() found no healthy baseline to seed", () => {
    // If WangPet boots while settings.json is already unhealthy (fresh install,
    ...
    // event, otherwise WangPet hooks would never get reinstalled.
    const { watcher, timers, syncCalls, getWatcher, setSettingsRaw } = makeWatcher({
      initialSettingsRaw: '{"hooks":{}}',
    });
  ...
  it("respects ctx.suspiciousShrinkRatio and ctx.suspiciousKeyLossThreshold tuning", () => {
    ...
    setSettingsRaw(SETTINGS_AFTER_USER_REMOVES_WANGPET_AND_ONE_THIRD_PARTY_HOOK);
    getWatcher().emitChange("settings.json");
    timers.flush();
  ...
  it("seeds the baseline on start so the very first watcher event can trip the guard", () => {
    // Cold start: WangPet just synced healthy hooks, then an external CLI minimizes
  ...
  it("treats non-object JSON payloads (null, array) as unparseable and allows resync", () => {
    ...
    // skipping resync there would leave WangPet unable to recover its own hooks.

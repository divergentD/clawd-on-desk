...
function tmpDir(label) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), `wang-pet-tg-store-${label}-`));
  return {
    dir,
...
test("parseTokenFromEnvFileText accepts the standard WANGPET_TG_BOT_TOKEN line", () => {
  assert.equal(parseTokenFromEnvFileText(`WANGPET_TG_BOT_TOKEN=${VALID}\n`), VALID);
  assert.equal(parseTokenFromEnvFileText(`  WANGPET_TG_BOT_TOKEN = ${VALID}  \n`), VALID);
});
...
test("parseTokenFromEnvFileText rejects malformed contents", () => {
  ...
  assert.equal(parseTokenFromEnvFileText("WANGPET_TG_BOT_TOKEN=garbage"), null);
});

test("buildEnvFileText produces the line the sidecar expects", () => {
  assert.equal(buildEnvFileText(VALID), `WANGPET_TG_BOT_TOKEN=${VALID}\n`);
});
...
test("envFileTokenStore.getToken reads the file and parses", async (t) => {
  ...
  t.after(tmp.cleanup);
  fs.writeFileSync(tmp.file, `WANGPET_TG_BOT_TOKEN=${VALID}\n`);
...
test("Source invariant: src/telegram-token-store.js never references process.env.WANGPET_TG_BOT_TOKEN", () => {
  ...
  assert.equal(
    source.includes("process.env.WANGPET_TG_BOT_TOKEN"),
    false,
    "token store must not read WANGPET_TG_BOT_TOKEN from process.env",
  );
  assert.equal(
    /process\.env\s*\.\s*WANGPET_TG_BOT_TOKEN/.test(source),
    false,
  );
});

...
describe("hit geometry", () => {
  ...
  it("derives image sizing from object fit for WangPet drag svg", () => {
    const rect = hitGeometry.getAssetRectScreen(WangPet, bounds, null, "wang-pet-react-drag.svg");
    approx(rect.x, -30.5);
    approx(rect.y, -53.6);
    approx(rect.w, 261);
    approx(rect.h, 261);
  ...
  it("derives the visible content rect from contentBox geometry", () => {
    const artRect = hitGeometry.getAssetRectScreen(WangPet, bounds, "idle", "wang-pet-idle-follow.svg");
    const expected = visibleContentRect(WangPet, artRect);
    const actual = hitGeometry.getContentRectScreen(WangPet, bounds, "idle", "wang-pet-idle-follow.svg");

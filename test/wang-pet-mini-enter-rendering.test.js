...
const WANGPET_THEME_PATH = path.join(REPO_ROOT, "themes", "wang-pet", "theme.json");
const WANGPET_SVG_DIR = path.join(REPO_ROOT, "assets", "svg");
...
function miniStateSvg(theme, state) {
  ...
  assert.ok(Array.isArray(files), `WangPet ${state} should map to a file list`);
  assert.strictEqual(files.length, 1, `WangPet ${state} should map to one SVG file`);
  assert.match(files[0], /\.svg$/i, `WangPet ${state} should use an SVG asset`);
  assert.ok(!files[0].includes("/") && !files[0].includes("\\"), `${state} asset should stay in assets/svg`);
  return files[0];
}

describe("WangPet mini enter SVG rendering", () => {
  it("does not force crisp root rendering on rotated mini enter assets", () => {
    const theme = JSON.parse(fs.readFileSync(WANGPET_THEME_PATH, "utf8"));

    for (const state of MINI_ENTER_STATES) {
      const file = miniStateSvg(theme, state);
      const root = readRootSvgTag(path.join(WANGPET_SVG_DIR, file));

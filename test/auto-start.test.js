...
test("auto-start exits without launching when WangPet is already listening", async () => {
  const calls = [];

  await new Promise((resolve) => {
    main({
      discoverwangpetPort(options, callback) {
...
test("waitForwangpetPort gives up after the startup deadline", async () => {
  const calls = [];
  let now = 0;

...
function tempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "wang-pet-tg-approval-"));
  tempDirs.push(dir);
  return dir;
}
...
test("writeTokenEnvFile tightens an existing token file on POSIX", { skip: process.platform === "win32" }, () => {
  ...
  assert.equal(fs.readFileSync(filePath, "utf8"), `WANGPET_TG_BOT_TOKEN=${token}\n`);
});
...
test("tokenStatus checks file presence without reading the token file", () => {
  ...
  const status = settings.tokenStatus({
    fs: fakeFs,
    filePath: "C:\\Users\\me\\AppData\\Roaming\\wang-pet\\telegram-approval.env",
  });
  ...
  assert.deepEqual(calls, [
    ["existsSync", "C:\\Users\\me\\AppData\\Roaming\\wang-pet\\telegram-approval.env"],
    ["statSync", "C:\\Users\\me\\AppData\\Roaming\\wang-pet\\telegram-approval.env"],
  ]);
});
...
test("readMaskedBotToken returns only the masked preview, not the raw token", () => {
  ...
  const token = "123456:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi_jklmnop";
  fs.writeFileSync(filePath, `WANGPET_TG_BOT_TOKEN=${token}\n`, "utf8");
...
test("tokenStatus ignores process.env.WANGPET_TG_BOT_TOKEN — file is the only signal", () => {
  ...
  // never has a route into WangPet's main process.
  const fakeFs = { existsSync: () => false, statSync: () => ({ mtimeMs: 0 }) };
  const status = settings.tokenStatus({
    fs: fakeFs,
    filePath: "/nonexistent/telegram-approval.env",
    env: { WANGPET_TG_BOT_TOKEN: "123456:ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghi" },
  });
...
test("invariant: WangPet source never reads process.env.WANGPET_TG_BOT_TOKEN", () => {
  // The bot token is only allowed to live at userData/telegram-approval.env on
  // disk. Any code that reads process.env.WANGPET_TG_BOT_TOKEN pulls the token
  // string into WangPet's main process, defeating that invariant. This grep
  // test fails loudly if a future refactor re-introduces the read.
  //
  // Note: the literal string "WANGPET_TG_BOT_TOKEN" is allowed to appear in
  ...
  // process.env access to that specific name in WangPet's own code.
  ...
  const offenders = [];
  const needle = "process.env.WANGPET_TG_BOT_TOKEN";

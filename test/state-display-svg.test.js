...
describe("display_svg session hints (updateSession path)", () => {
  ...
  it("uses allowlisted display_svg for working state", () => {
    api.updateSession("c1", "working", "PreToolUse", baseOpts({ displayHint: "wang-pet-working-building.svg" }));
  ...
  it("picks the most recently updated session among working sessions", async () => {
    api.updateSession("a", "working", "PreToolUse", baseOpts({ cwd: "/a", displayHint: "wang-pet-working-building.svg" }));
    await new Promise((r) => setTimeout(r, 5));
    api.updateSession("b", "working", "PostToolUse", baseOpts({ cwd: "/b", displayHint: "wang-pet-idle-reading.svg" }));
    assert.strictEqual(api.getSvgOverride("working"), "wang-pet-idle-reading.svg");
  });
  ...
  it("applies thinking hint for thinking state", () => {
    api.updateSession("c1", "thinking", "AfterAgentThought", baseOpts({ displayHint: "wang-pet-working-thinking.svg" }));
    assert.strictEqual(api.getSvgOverride("thinking"), "wang-pet-working-thinking.svg");
  });
});
